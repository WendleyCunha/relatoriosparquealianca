...
# --- CONEXÃO COM FIRESTORE (Mantido original) ---
def inicializar_db():
...
# --- LÓGICA DO MÊS (Mantido original) ---
...
def limpar_formulario():
    ...
    # Limpa os widgets diretamente via keys
    st.session_state.txt_nome = ""
    st.session_state.chk_part = False
    st.session_state.num_estudos = 0
    st.session_state.num_horas = 0
    st.session_state.txt_obs = ""

# --- INTERFACE ---
def main():
    # Inicialização de estados (Mantido original)
    ...
    if st.session_state.enviado:
        ...
        with placeholder.container():
            ...
            st.write("")
            if st.button("VOLTAR PARA O RELATÓRIO", use_container_width=True):
        ...
        with placeholder.container():
            ...
            with col1: st.write("Estudos bíblicos")
            with col2: estudos = st.number_input("Estudos", min_value=0, step=1, value=st.session_state.estudos_val, label_visibility="collapsed", key="num_estudos")
                
            col3, col4 = st.columns([3, 1])
            with col3: st.write("Horas (Pioneiros/Missionários)")
            with col4: horas = st.number_input("Horas", min_value=0, step=1, value=st.session_state.horas_val, label_visibility="collapsed", key="num_horas")
            ...
            if st.button("ENVIAR RELATÓRIO", use_container_width=True):
                if not nome.strip():
                    ...
                    # LOGICA DE GRAVAÇÃO ORIGINAL PRESERVADA 100%
                    dados_final = {
                        "nome": nome,
                    ...
                    if salvar_relatorio(dados_final):
                        ...
                        # Chame a limpeza antes de mudar para a tela de sucesso
                        limpar_formulario()
